...
class CircuitBreaker:
    ...
    def call(self, func, *args, **kwargs):
        ...
        with self.lock:
            if self.state == 'OPEN':
                oktime = time.time() - (self.last_failure_time or 0)
                if oktime > self.recovery_timeout:
                    self.state = 'HALF_OPEN'
                else:
                    raise CircuitBreakerOpenException("Circuito aperto: API Rifiutata.")
        try:
            ris = func(*args, **kwargs)
